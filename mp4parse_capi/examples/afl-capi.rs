                ...
                if rv == Mp4parseStatus::Ok {
                    ...
                    match info.track_type {
                        Mp4parseTrackType::Video => {
                            ...
                            if rv == Mp4parseStatus::Ok {
                                println!(
                                    "  video: display={}x{} rotation={}",
                                    video.display_width, video.display_height, video.rotation
                                );
                                for i in 0..video.sample_info_count as isize {
                                    let info = &*video.sample_info.offset(i);
                                    println!(
                                        "    sample info[{}]: codec={:?} image={}x{}",
                                        i, info.codec_type, info.image_width, info.image_height
                                    );
                                }
                            }
                        }
