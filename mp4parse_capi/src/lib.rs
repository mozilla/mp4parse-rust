...
//!     assert_eq!(rv, mp4parse_capi::MP4PARSE::OK);
...
#[no_mangle]
pub unsafe extern fn mp4parse_read(parser: *mut mp4parse_parser) -> MP4PARSE {
    // Validate arguments from C.
    if parser.is_null() || (*parser).poisoned() {
        return MP4PARSE::ERROR_BADARG;
    }
    ...
    match r {
        ...
        Err(Error::NoMoov) | Err(Error::InvalidData(_)) => {
            ...
            MP4PARSE::ERROR_INVALID
        }
        Err(Error::Unsupported(_)) => MP4PARSE::ERROR_UNSUPPORTED,
        Err(Error::UnexpectedEOF) => MP4PARSE::ERROR_EOF,
        Err(Error::Io(_)) => {
...
#[no_mangle]
pub unsafe extern fn mp4parse_get_track_count(parser: *const mp4parse_parser, count: *mut u32) -> MP4PARSE {
    // Validate arguments from C.
...
#[no_mangle]
pub unsafe extern fn mp4parse_get_track_info(parser: *mut mp4parse_parser, track_index: u32, info: *mut mp4parse_track_info) -> MP4PARSE {
    ...
    *info = Default::default();
    ...
    info.track_type = match context.tracks[track_index].track_type {
        ...
        TrackType::Unknown => return MP4PARSE::ERROR_UNSUPPORTED,
    };
    ...
    if let (Some(track_timescale),
        ...
        let media_time =
            match track.media_time.map_or(Some(0), |media_time| {
                ...
                None => return MP4PARSE::ERROR_INVALID,
            };
        let empty_duration =
...
#[no_mangle]
pub unsafe extern fn mp4parse_get_track_audio_info(parser: *mut mp4parse_parser, track_index: u32, info: *mut mp4parse_track_audio_info) -> MP4PARSE {
    if parser.is_null() || info.is_null() || (*parser).poisoned() {
        return MP4PARSE::ERROR_BADARG;
    }
    ...
    *info = Default::default();
    ...
    match track.track_type {
        TrackType::Audio => {}
        _ => return MP4PARSE::ERROR_INVALID,
    };
...
#[no_mangle]
pub unsafe extern fn mp4parse_get_track_video_info(parser: *mut mp4parse_parser, track_index: u32, info: *mut mp4parse_track_video_info) -> MP4PARSE {
    if parser.is_null() || info.is_null() || (*parser).poisoned() {
        return MP4PARSE::ERROR_BADARG;
    }
...
#[no_mangle]
pub unsafe extern fn mp4parse_get_indice_table(parser: *mut mp4parse_parser, track_id: u32, indices: *mut mp4parse_byte_data) -> MP4PARSE {
    if parser.is_null() || (*parser).poisoned() {
        return MP4PARSE::ERROR_BADARG;
    }
...
#[no_mangle]
pub unsafe extern fn mp4parse_get_fragment_info(parser: *mut mp4parse_parser, info: *mut mp4parse_fragment_info) -> MP4PARSE {
    if parser.is_null() || info.is_null() || (*parser).poisoned() {
        return MP4PARSE::ERROR_BADARG;
    }
...
#[no_mangle]
pub unsafe extern fn mp4parse_is_fragmented(parser: *mut mp4parse_parser, track_id: u32, fragmented: *mut u8) -> MP4PARSE {
    if parser.is_null() || (*parser).poisoned() {
        return MP4PARSE::ERROR_BADARG;
    }
    ...
    match iter.find(|track| track.track_id == Some(track_id)) {
        ...
        Some(_) => {},
        None => return MP4PARSE::ERROR_BADARG,
    }
...
#[no_mangle]
pub unsafe extern fn mp4parse_get_pssh_info(parser: *mut mp4parse_parser, info: *mut mp4parse_pssh_info) -> MP4PARSE {
    if parser.is_null() || info.is_null() || (*parser).poisoned() {
        return MP4PARSE::ERROR_BADARG;
    }
...
fn get_track_count_null_parser() {
    unsafe {
        ...
        assert_eq!(rv, MP4PARSE::ERROR_BADARG);
...
fn arg_validation() {
    unsafe {
        ...
        assert_eq!(MP4PARSE::ERROR_BADARG, mp4parse_read(std::ptr::null_mut()));
        ...
        assert_eq!(MP4PARSE::ERROR_BADARG, mp4parse_get_track_info(std::ptr::null_mut(), 0, &mut dummy_info));
        ...
        assert_eq!(MP4PARSE::ERROR_BADARG, mp4parse_get_track_video_info(std::ptr::null_mut(), 0, &mut dummy_video));

        let mut dummy_audio = Default::default();
        assert_eq!(MP4PARSE::ERROR_BADARG, mp4parse_get_track_audio_info(std::ptr::null_mut(), 0, &mut dummy_audio));
    }
}
...
fn arg_validation_with_parser() {
    unsafe {
        ...
        assert_eq!(MP4PARSE::ERROR_IO, mp4parse_read(parser));

        // The parser is now poisoned and unusable.
        assert_eq!(MP4PARSE::ERROR_BADARG,  mp4parse_read(parser));

        // Null info pointers are an error.
        assert_eq!(MP4PARSE::ERROR_BADARG, mp4parse_get_track_info(parser, 0, std::ptr::null_mut()));
        assert_eq!(MP4PARSE::ERROR_BADARG, mp4parse_get_track_video_info(parser, 0, std::ptr::null_mut()));
        assert_eq!(MP4PARSE::ERROR_BADARG, mp4parse_get_track_audio_info(parser, 0, std::ptr::null_mut()));
        ...
        assert_eq!(MP4PARSE::ERROR_BADARG, mp4parse_get_track_info(parser, 0, &mut dummy_info));
        ...
        assert_eq!(MP4PARSE::ERROR_BADARG, mp4parse_get_track_video_info(parser, 0, &mut dummy_video));

        let mut dummy_audio = Default::default();
        assert_eq!(MP4PARSE::ERROR_BADARG, mp4parse_get_track_audio_info(parser, 0, &mut dummy_audio));
...
fn arg_validation_with_data() {
    unsafe {
        ...
        assert_eq!(MP4PARSE::OK, mp4parse_read(parser));

        let mut count: u32 = 0;
        assert_eq!(MP4PARSE::OK, mp4parse_get_track_count(parser, &mut count));
        assert_eq!(2, count);
        ...
        assert_eq!(MP4PARSE::OK, mp4parse_get_track_info(parser, 0, &mut info));
        ...
        assert_eq!(MP4PARSE::OK, mp4parse_get_track_info(parser, 1, &mut info));
        ...
        assert_eq!(MP4PARSE::OK, mp4parse_get_track_video_info(parser, 0, &mut video));
        ...
        assert_eq!(MP4PARSE::OK, mp4parse_get_track_audio_info(parser, 1, &mut audio));
        ...
        assert_eq!(MP4PARSE::ERROR_BADARG, mp4parse_get_track_info(parser, 3, &mut info));
        assert_eq!(info.track_type, MP4PARSE_TRACK_TYPE::VIDEO);
        ...
        assert_eq!(MP4PARSE::ERROR_BADARG, mp4parse_get_track_video_info(parser, 3, &mut video));
        ...
        let mut audio = Default::default();
        assert_eq!(MP4PARSE::ERROR_BADARG, mp4parse_get_track_audio_info(parser, 3, &mut audio));
